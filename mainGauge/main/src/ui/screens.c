    ...
    lv_obj_set_style_bg_img_src(obj, &img_main_gauge_bg, LV_PART_MAIN | LV_STATE_DEFAULT);
    {
        lv_obj_t *parent_obj = obj;
        {
            ...
            lv_obj_set_size(obj, 800, 800);
            {
                ...
                state->scale = scale;
                lv_meter_set_scale_ticks(obj, scale, 0, 0, 0, lv_color_black());
                lv_meter_set_scale_major_ticks(obj, scale, 0, 0, 0, lv_color_black(), 0);

                // Disable labels entirely
                lv_obj_set_style_text_font(obj, NULL, LV_PART_TICKS);
                lv_obj_set_style_text_opa(obj, 0, LV_PART_TICKS);

                // Hide scale items
                lv_obj_set_style_opa(obj, 0, LV_PART_ITEMS);
