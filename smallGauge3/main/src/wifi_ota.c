...
#define PROV_SSID "smallGauge3"
#define HOSTNAME  "smallGauge3"
...
static void start_webserver(void)
{
    if (server == NULL) {
        ...
        // 🔧 increase stack size for OTA uploads
        config.stack_size = 8192;   // or 10240 if you want extra headroom
                               ...
                               int32_t event_id, void *event_data)
{
    if (event_base == WIFI_EVENT) {
        if (event_id == WIFI_EVENT_STA_START) {
            if (s_current_wifi_state == APP_WIFI_STATE_CONNECTED) esp_wifi_connect();
        ...
        if (event_id == IP_EVENT_STA_GOT_IP) {
            if (s_current_wifi_state == APP_WIFI_STATE_CONNECTED) start_webserver();
        }
    } else if (event_base == WIFI_PROV_EVENT) {
        if (event_id == WIFI_PROV_CRED_SUCCESS) {
            ESP_LOGI(TAG, "Provisioning successful! Rebooting...");
...
void set_wifi_mode(app_wifi_state_t new_state) {
    if (s_current_wifi_state == new_state) return;
    ...
    switch (new_state) {
        ...
        case APP_WIFI_STATE_PROVISIONING: {
            ...
            ESP_ERROR_CHECK(wifi_prov_mgr_start_provisioning(WIFI_PROV_SECURITY_1, PROV_POP, PROV_SSID, NULL));
        } break;
...
void init_wifi_state_machine(void) {
    ...
    if (!provisioned) set_wifi_mode(APP_WIFI_STATE_PROVISIONING);
    else set_wifi_mode(g_ota_mode_enabled ? APP_WIFI_STATE_CONNECTED : APP_WIFI_STATE_ESP_NOW_ONLY);
}

void enter_ota_mode(void) { g_ota_mode_enabled = true; set_wifi_mode(APP_WIFI_STATE_CONNECTED); }
void exit_ota_mode(void) { g_ota_mode_enabled = false; set_wifi_mode(APP_WIFI_STATE_ESP_NOW_ONLY); }

app_wifi_state_t get_wifi_state(void) { return s_current_wifi_state; }

const char* wifi_state_to_str(app_wifi_state_t state) {
    const char *names[] = { "INIT", "ESP_NOW_ONLY", "PROVISIONING", "CONNECTED" };
    if (state >= 0 && state <= 3) return names[state];
    return "UNKNOWN";
}
