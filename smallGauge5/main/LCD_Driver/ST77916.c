...
// Add this near the top of ST77916.c
extern bool notify_lvgl_flush_ready(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_io_event_data_t *edata, void *user_ctx);
extern lv_disp_drv_t disp_drv; // Extern reference to your global LVGL display driver struct

static const char *TAG_LCD = "ST77916";

static SemaphoreHandle_t te_sem = NULL;

// This ISR fires the moment the ST77916 screen begins its Vertical Blanking period
...
// Call this function inside LCD_Init() to configure GPIO 18
void Manual_TE_Pin_Init(void)
{
  ...
  gpio_config_t io_conf = {
      ...
      .pull_up_en = GPIO_PULLUP_ENABLE, // Keep line stable
      .pull_down_en = GPIO_PULLDOWN_DISABLE,
      .intr_type = GPIO_INTR_NEGEDGE // Fire on falling edge (start of V-Blank)
  };
  gpio_config(&io_conf);

  // Install GPIO ISR service if not already installed elsewhere in your app
  gpio_install_isr_service(0);
  gpio_isr_handler_add(ESP_PANEL_LCD_SPI_IO_TE, lcd_te_isr_handler, NULL);
}

// A global function that LVGL can call to wait for the screen refresh boundary
void wait_for_vsync(void)
{
  if (te_sem != NULL)
  {
    // Clear out any old pending triggers, then wait for a fresh V-Blank hit
    xSemaphoreTake(te_sem, 0);
    xSemaphoreTake(te_sem, pdMS_TO_TICKS(20)); // Max wait 20ms (~60Hz timeout fallback)
  }
}
...
static const st77916_lcd_init_cmd_t vendor_specific_init_new[] = {
    ...
    // Add this right before {0x29, (uint8_t []){0x00}, 1, 0} in your array if tearing persists:
    {0x35, (uint8_t[]){0x00}, 1, 0}, // 0x35 is the standard MIPI command for TE ON (V-Blanking only)
...
{
  ST77916_Init();
  Manual_TE_Pin_Init(); // Initialize GPIO 18 for TE signal
  Backlight_Init();
  Touch_Init();
}
...
int QSPI_Init(void)
{
  ...
  // =================================================================
  // PHASE 1: Create a safe, temporary 3MHz handle JUST to read register 0x04
  // =================================================================
  esp_lcd_panel_io_spi_config_t io_config = {
      .cs_gpio_num = ESP_PANEL_LCD_SPI_IO_CS,
      .dc_gpio_num = -1,
      .spi_mode = ESP_PANEL_LCD_SPI_MODE,
      .pclk_hz = 3 * 1000 * 1000, // Safe 3MHz read clock
  ...
  {
    printf("Failed to set temporary LCD communication parameters -- SPI\r\n");
    return 0;
  }
  ...
  // Cleanly delete the temporary 3MHz handle so it doesn't conflict or leak memory
  esp_lcd_panel_io_del(io_handle);

  // =================================================================
  // PHASE 2: Recreate the handle at full operational speed (80MHz)
  // =================================================================

  // 1. Reset the callback pointers in the config so the driver
  //    doesn't think we are trying to set them twice.
  io_config.pclk_hz = ESP_PANEL_LCD_SPI_CLK_HZ;
  io_config.on_color_trans_done = NULL; // Clear for the new handle
  io_config.user_ctx = NULL;            // Clear for the new handle

  if (esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)ESP_PANEL_HOST_SPI_ID_DEFAULT, &io_config, &io_handle) != ESP_OK)
  {
    printf("Failed to set production LCD communication parameters -- SPI\r\n");
    return 0;
  }
  printf("Production LCD communication parameters set successfully -- SPI\r\n");

  // 2. Register the callback ONLY ONCE here.
  esp_lcd_panel_io_callbacks_t io_cbs = {
      .on_color_trans_done = notify_lvgl_flush_ready};
  esp_lcd_panel_io_register_event_callbacks(io_handle, &io_cbs, &disp_drv);

  // =================================================================
  // PHASE 3: Apply Panel Config and Initialize
  // =================================================================
  ...
  // Give the display's internal LDOs and charge pumps time to stabilize
  // Uniform banding often occurs when the charge pump hasn't finished ramping
  vTaskDelay(pdMS_TO_TICKS(200));

  esp_lcd_panel_init(panel_handle);
  // Important: Wait for the panel to finish its internal startup sequence
